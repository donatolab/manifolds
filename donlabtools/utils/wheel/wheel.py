...
class Wheel:
    ...
    def __init__(self):
        ...
        self.wheel.length_of_click = (
            2 * np.pi * self.wheel.radius / self.wheel.clicks_per_rotation
        )
    ...
    #
    def load_track(self, session_part: str = None):
        wheel_fname = "wheel.npy"
        galvo_fname = "2p_galvo_trigger.npy"
        triggers_fname_out = "triggers.npy"

        if session_part:
            wheel_fname = f"{session_part}_{wheel_fname}"
            galvo_fname = f"{session_part}_{galvo_fname}"
            triggers_fname_out = f"{session_part}_{triggers_fname_out}"

        self.fname = os.path.join(self.root_dir, wheel_fname)
        ...
        # get time on track
        times = (
            np.arange(self.track.rotary_binarized.values.shape[0]) / self.sample_rate
        )
        ...
        self.track.distances.values = (
            self.track.positions.values * self.wheel.length_of_click
        )
        self.track.distances.times = self.track.rotary_binarized.times

        #
        self.track.fname_galvo_trigger = os.path.join(self.root_dir, galvo_fname)
        ...
        fname_out = os.path.join(os.path.split(self.fname)[0], triggers_fname_out)
        try:
    ...
    def compute_xy(self):
        ...
        x, y = convert_polar_to_xy(track_pos, sample_rate)
    ...
    def plot_track(self, obj, ax=None, c="black", label=None):
        ...
        t = obj.times
        vals = obj.values
        ax.plot(t, vals, c=c, label=label)
    ...
    def get_indexes_quiescent_periods(self):
        ...
        print(
            "   Queiscent periods (seconds): ", idx.shape[0] / self.imaging_sample_rate
        )

        return idx

    #
    def compute_velocity(self, session_part: str = None):
        fname = "velocity.npy"
        if session_part:
            fname = f"{session_part}_{fname}"
        fname_out = os.path.join(os.path.split(self.fname)[0], fname)
        ...
        if os.path.exists(fname_out):
            ...
            #
            click_distance = (
                2 * np.pi * self.wheel.radius
            ) / self.wheel.clicks_per_rotation
            ...
            for id_ in tqdm(idx):
                ...
                v[id_] = (click * click_distance) / (
                    (id_ - last_time_idx) / self.sample_rate
                )
            ...
            # if only zeros create dummy array
            if len(v2) == 0:
                v2 = np.array([0])
                ...
                vel_times = np.int32(
                    np.delete(self.track.rotary_binarized.times, idx2)
                    * self.sample_rate
                )
            ...
            for k in trange(
                1, vel_times.shape[0], 1, desc="refilling stationary times"
            ):
            ...
            if v2.shape[0] != 0:
                F = interp1d(vel_times, v2, fill_value="extrapolate")

                print(
                    "self.track.galvo_triggers.times: ", self.track.galvo_triggers.times
                )
                print("vel_times:", vel_times)
