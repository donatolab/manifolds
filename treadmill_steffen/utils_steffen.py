...
def show_cell_session_averages(experiment_id,
                               session_ids,
                               up_phase=True,
                               reorder=True):
    # load default names
    ...
    for session_id in session_ids:
        ...
        #
        print('')
        print("pos track: ", pos_track, pos_track.shape)
        ...
        for cell_id in tqdm(cell_ids):
            ...
            #
            f0 = np.median(ca[cell_id])
            if f0 != 0:

                #
                for k in range(idx_track.shape[0] - 1):
                    temp = ca[cell_id, idx_track[k]].copy()
                    temp = temp - f0
...
def show_scatter_plots(imgs,
    ...
    plt.xlim(0,180)
    plt.title("Pcorr >"+str(corr_thresh) + ", # of cells with pval <"+str(pval_thresh)+" : " +str(len(corrs)))
    plt.legend()
    plt.suptitle(names[session_ids[0]] +  " vs " + names[session_ids[1]]+ " , # cells: "+str(len(corrs)))
    plt.show(block=False)
